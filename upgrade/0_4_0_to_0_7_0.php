...
function upgrade_reportit_0_4_0_to_0_7_0(){
    ...
    if (!in_array('cf', $columns)) {
        db_execute("ALTER TABLE reportit_cache_measurands
                    ADD `cf` int(11) NOT NULL default '1'");
    }
    ...
    $sql = "ALTER TABLE reportit_reports
    ...
    $sql = "ALTER TABLE reportit_templates
            MODIFY `description`            varchar(255)        NOT NULL default '',
    ...
    $sql = "ALTER TABLE reportit_measurands
            MODIFY `template_id`            int(11)             NOT NULL default '0',
            MODIFY `description`            varchar(255)        NOT NULL default '',
    ...
    $sql = "ALTER TABLE reportit_variables
            MODIFY `template_id`            int(11)             NOT NULL default '0',
            MODIFY `abbreviation`           varchar(255)        NOT NULL default '',
    ...
    $sql = "ALTER TABLE reportit_rvars
            MODIFY `template_id`            int(11)             NOT NULL default '0',
    ...
    $sql = "ALTER TABLE reportit_cache_reports
            MODIFY `ds_description`         varchar(5000)       NOT NULL default '',
    ...
    if(is_array($template_configs)) {
	    ...
	    if(!isset($template['cf'])) continue;
            ...
            $sql = "SELECT id, data_source_name FROM data_template_rrd
                    WHERE local_data_id=0 AND data_template_id = $data_template_id";
            $names = db_fetch_assoc($sql);

            /* load defined aliases */
            $ds_names = db_fetch_cell("SELECT data_template_alias FROM reportit_templates WHERE id = $template_id");
            $ds_names = unserialize($ds_names);
            strip_slashes($ds_names);
