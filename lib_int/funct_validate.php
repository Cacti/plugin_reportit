...
function last_error($errno, $errstr) {
    global $error;
    $error = array( 'Number'  => $errno,'Message' => $errstr);
}
...
function validate_calc_formula($calc_formula, $calc_intersizes, $calc_var_names, $calc_data_query_variables) {
    ...
    if(strlen($debug) != 0) return "Invalid characters: $debug";

    //Check the number of parenthensis;
    if(substr_count($calc_formula, '(') != substr_count($calc_formula, ')')) return "Missing parenthensis:  <span style='color:blue'>$calc_formula<span>";

    //Check if the formula begins or ends with an operator
    if(in_array($calc_formula[0], $valids['operators']['S'])) return "Formula begins with an operator.";
    if(in_array($calc_formula[strlen($calc_formula)-1], $valids['operators']['S'])) return "Formula ends with an operator.";
...
function safeguard_xss($array = '', $default_tags = false, $allowable_tags = false) {
    global $config;
    ...
    if (is_array($allowable_tags)) {
        foreach($allowable_tags as $tag) $exception .= ' �' . $tag . '�,';
        $exception = substr($exception, 0, strlen($exception)-1);
    }elseif (is_string($allowable_tags)) {
        $exception = $allowable_tags;
    }

    if ($default_tags !== false) {
        $exception .= ($exception == '') ? $default
                                         : ", $default";
    }

    foreach($array as $key => $value) {
        if($value != strip_tags($value, $exception) || $key != strip_tags($key, $exception)) {
			top_header();
...
function input_validate_input_whitelist($value, $valid_list, $undefined=false, $header=true){
	if($value == false & $undefined == true) return;
...
function input_validate_input_limits($value, $lower_limit, $upper_limit, $inside=true, $header=true ){
	if($inside) {
		if ($value<$lower_limit & $value>$upper_limit) die_html_custom_error('', $header);
	}else {
		if ($value>$lower_limit & $value<$upper_limit) die_html_custom_error('', $header);
	}
}
...
function validate_uploaded_template(){
    ...
    if (isset($_FILES['file'])) {
        ...
        if ($_FILES['file']['error'] != 0) {
            ...
            {
                case 1:
                    session_custom_error_message("file", "The file is to big.",false);
                    break;
                case 2:
                    session_custom_error_message("file", "The file is to big.",false);
                    break;
                case 3:
                    session_custom_error_message("file", "Incomplete file transfer.",false);
                    break;
                case 4:
                    session_custom_error_message("file", "No file uploaded.",false);
                    break;
                case 6:
                    session_custom_error_message("file", "Temporary folder missing.",false);
                    break;
                case 7:
                    session_custom_error_message("file", "Failed to write file to disk",false);
                    break;
                case 8:
                    session_custom_error_message("file", "File upload stopped by extension",false);
                    break;
             }
        ...
        if($_FILES['file']['type'] != 'text/xml') {
            session_custom_error_message("file", "Invalid file extension.",false);
            return false;
        }
    ...
    if(!is_array($data)) {
        session_custom_error_message("file", "Unable to parse template file.");
        return false;
    }else {
        ...
        if($hash == false | $hash !== md5($checksum) | $valid === false) {
            session_custom_error_message("file", "Checksum error.", false);
            return false;
        }
        ...
        $sql = "SELECT data_source_name FROM data_template_rrd
                WHERE local_data_id=0 AND data_template_id = $data_template_id";
        ...
        if(in_array($template_ds_names, $ds_names) === false) {
            ...
            $sql = "SELECT a.id, b.name FROM
                        (
                            SELECT data_template_id AS id, COUNT(*) as `cnt` FROM `data_template_rrd`
