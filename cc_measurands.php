...
function standard() {
	...
	$header_label	= __("Measurands [Template: <a style='color:yellow' class='pic' href='cc_templates.php?action=template_edit&id=" . get_request_var('id') . "'>%s</a>] [%d]", $template_name, $number_of_measurands);

	html_start_box($header_label, '100%', '', '2', 'center', 'cc_measurands.php?action=measurand_edit&template_id=' . get_request_var('id'));

	$display_text = array(
		__('Name'),
		__('Abbreviation'),
		__('Unit'),
		__('Consolidation Function'),
		__('Visible'),
		__('Separate'),
		__('Calculation Formula')
	);
	...
	if (sizeof($measurands_list)) {
		foreach($measurands_list as $measurand) {
			form_alternate_row_color('line' . $measurand['id'], true);

			form_selectable_cell("<a class='linkEditMain' href='cc_measurands.php?action=measurand_edit&id=" . $measurand['id'] . "'>" . $measurand['description'] . '</a>', $measurand['id']);
			...
			form_selectable_cell(html_checked_with_arrow($measurand['visible']), $measurand['id']);
			form_selectable_cell(html_checked_with_arrow($measurand['spanned']), $measurand['id']);
			form_selectable_cell($measurand['calc_formula'], $measurand['id']);
			form_checkbox_cell($measurand['id']);
			form_end_row();
		}
	}else {
		print '<tr><td colspan="8"><em>' . __('No Measurands Found') . '</em></td></tr>';
	}
	...
	draw_actions_dropdown($measurand_actions, 'cc_templates.php');
}
...
function form_save() {
	...
	if ($count != 0) {
		session_custom_error_message('measurand_abbreviation', __('Duplicate abbreviation'));
	}
...
;
			...
			if ($count != 0) {
				session_custom_error_message('measurand_spanned', __('Interim results are used by other measurands.'));
			}
		}
	}
	...
	if (is_error_message()) {
		header('Location: cc_measurands.php?header=false&action=measurand_edit&id=' . get_request_var('id') . '&template_id=' . get_request_var('template_id'));
	} else {
		//Save data
		...
		header('Location: cc_measurands.php?header=false&id=' . get_request_var('template_id'));
		raise_message(1);
	}
}
...
function measurand_edit() {
	...
	if (!isempty_request_var('id')) {
		...
		$header_label   = __('Measurand Configuration [edit: %s]' . $measurand_data['description']);
	}else {
		$header_label   = __('Measurand Configuration [new]');
	}
	...
	$form_array = array(
		...
		'measurand_description'	=> array(
			...
			'description'		=> __('The explanation given to this measurand.<br>Available as legend and tooltip.'),
			'method'			=> 'textbox',
			'max_length'		=> '100',
			'value'				=> (isset($measurand_data['description']) ? $measurand_data['description'] : '')
		),
		'measurand_abbreviation'=> array(
			'friendly_name'		=> __('Abbreviation'),
			'description'		=> __('Define a unique abbreviation for this<br> measurand with max. 4 letters/numbers.'),
			'method'			=> 'textbox',
			'max_length'		=> '4',
			'value'				=> (isset($measurand_data['abbreviation']) ? $measurand_data['abbreviation'] : '')
		),
		'measurand_unit'	=> array(
			'friendly_name'		=> __('Unit'),
			'description'		=> __('The unit given to this measurand.<br>e.g. \'Bits/s\''),
			'method'			=> 'textbox',
			'max_length'		=> '25',
		...
		'measurand_cf'		=> array(
			...
			'description'		=> __('The name of the consolidation function to define which CDPs should be read out.'),
			'method'			=> 'drop_array',
			'default'			=> '0',
		...
		'measurand_visible'	=> array(
			'friendly_name'		=> __('Visible'),
			'description'		=> __('Choose \'enable\' if this measurand should be shown in the report.'),
			'method'			=> 'custom',
		...
		'measurand_spanned'	=> array(
			'friendly_name'		=> __('Separate'),
			'description'		=> __('Choose \'enable\' if this measurand will only have one result<br>instead of one for every Round Robin Archive and it\'s result<br>should be displayed separately.'),
			'method'			=> 'custom',
		...
		'measurand_type'=> array(
			...
			'description'		=> __('Defines as what type the data should be treated as.'),
			'value'				=> (isset($measurand_data['data_type']) ? $measurand_data['data_type'] : '1' )
		),
		'measurand_precision'=> array(
			'friendly_name'		=> __('Precision'),
			'description'		=> __('Defines how many decimal digits should be displayed for floating-point numbers.'),
			'method'			=> 'drop_array',
			'array'				=> $precision,
			'value'				=> (isset($measurand_data['data_precision']) ? $measurand_data['data_precision'] : '2' )
		),
		'measurand_rounding'=> array(
			'friendly_name'		=> __('Prefixes'),
			'description'		=> __('Choose the type of prefix beeing used to format the result. With the use of decimal prefixes \'1024\' will be formatted to \'1.024k\' while the binary prefixes option returns \'1ki\'. Select \'off\' to display the raw data, here \'1024\'.'),
			'method'			=> 'drop_array',
			'array'				=> $rounding,
			'value'				=> (isset($measurand_data['rounding']) ? $measurand_data['rounding'] : '2' )
		),
		'measurand_header3'	=> array(
			'friendly_name'		=> __('Formula'),
			'method'			=> 'spacer'
		),
		'measurand_formula'	=> array(
			'friendly_name'		=> __('Calculation Formula'),
			'description'		=> __('This measurand\'s mathematical definition.<br>Use the operators and operands below.'),
			'method' 			=> 'custom',
	...
	desc[0]['description']	= '<?php print __('Returns the average of all measured values.');?>';
	...
	desc[1]['description']	= '<?php print __('Returns measured data\'s highest value.');?>';
	...
	desc[2]['description'] 	= '<?php print __('Returns measured data\'s lowest value.');?>';
	...
	desc[3]['description'] 	= '<?php print __('Returns the sum of all measured values.');?>';
	...
	desc[4]['description'] 	= '<?php print __('Returns the number of valid measured values. (excludes NaN\'s)');?>';
	...
	desc[5]['description'] 	= '<?php print __('Returns the gradient of a straight line by using linear regression. (trend analysis)');?>';
	...
	desc[6]['description'] 	= '<?php print __('Returns the last valid measured value of the reporting period. (excludes NaN\'s)');?>';
	...
	desc[8]['description'] 	= '<?php print __('Returns the xth percentile.');?>';
	...
	desc[9]['description'] 	= '<?php print __('Returns the duration over a defined threshold in percent.');?>';
	...
	desc[10]['description'] = '<?php print __('Returns the sum over a defined threshold.');?>';
	...
	desc[11]['description']	= '<?php print __('This variable contains the maximum bandwidth if a value for SNMP parameter \'ifspeed\' or \'ifHighSpeed\' is available.');?>';
	...
	desc[12]['description']	= '<?php print __('This variable contains the maximum value that has been defined for the specific data source item under \'Data Sources\'.');?>'
	...
	desc[13]['description'] = '<?php print __('The step is the distance of time between two measured values in seconds.');?>';
	...
	desc[14]['description'] = '<?php print __('Sum of NaN\'s during the reporting period. It\'s <b>NOT</b> differentiated in data sources!');?>';
	desc[14]['example']	= 'nan';
	...
	desc[15]['description'] = '<?php print __('Returns the integer value of any given float, string.');?>';
	...
	desc[16]['description'] = '<?php print __('Returns the rounded integer value of any given float, string.');?>';
	...
	desc[17]['description'] = '<?php print __('Returns the highest value of a given list of parameters.');?>';
	...
	desc[18]['description'] = '<?php print __('Returns the lowest value of a given list of parameters.');?>';
			...
			for(var i = 0; i < desc.length; i++) {
				if (desc[i]['name'] == name) {
					Tooltip.innerHTML =
						...
						+'<tr><td width="100"><?php print __('Name:');?></td><td>' + name + '</td></tr>'
						+'<tr><td><?php print __('Syntax:');?></td><td>' + desc[i]['syntax'] + '</td></tr>'
						+'<tr><td><?php print __('Parameters:');?></td><td>' + desc[i]['params'] + '</td></tr>'
						+'<tr><td valign="top"><?php print __('Description:');?></td><td>' + desc[i]['description'] + '</td></tr>'
						+'<tr><td align="left" valign="top"><?php print __('Example:');?></td><td>' + desc[i]['example'] + '</td></tr>'
						+'</table>';
...
function form_actions() {
	...
	if (isset_request_var('selected_items')) {
		...
		header('Location: cc_measurands.php?header=false&id=' . get_request_var('id'));
		exit;
	}
	...
	form_start('cc_measurands.php');

	if (get_request_var('drp_action') == '2') { //DELETE REPORT
		print "<tr class='odd'>
			<td class='textArea'>
				<p>" . __('Click \'Continue\' to Delete the following Measurands.  Notice: If there are no other Measurands left after this process, the Report Template will be locked automatically.') . '<p>';

		if(is_array($ds_list)) {
			print '<p>' . __('List of selected measurands:') . '</p>';
			print '<ul>';
			foreach($ds_list as $key => $value) {
				print '<li>' . __('Measurand: %s', $key) . '</li>';
			}
			print '</ul>';
		}
		print '</td>
		...
		if (!is_array($ds_list)) {
			print "<tr>
				<td class='textArea'>
					<span class='textError'>" . __('You must select at least one measurand.') . '</span>
				</td>
			</tr>';

			$save_html = "<input type='button' value='" . __('Cancel') . "' onClick='cactiReturnTo()'>";
		}else {
			$save_html = "<input type='button' value='" . __('Cancel') . "' onClick='cactiReturnTo()'>&nbsp;<input type='submit' value='" . __('Continue') . "' title='" . __('Delete Template Measurands') . "'>";
		}
	}
