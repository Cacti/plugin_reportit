 ...
 * @param int $group_id contains the id of the current group
 * @return an array with all possible round robin archives
 */
function get_possible_rra_names($group_id) {
    //Get all possible names of the RRAs for this type of template
    $names = array();
    $array = array();

    $names = db_fetch_assoc_prepared("SELECT DISTINCT (
		CASE
			WHEN data_source_alias != '' THEN data_source_alias ELSE data_source_name
		END) as name
		FROM plugin_reportit_data_source_items
		WHERE id != 0
 ...
 * @param int $group_id contains the id of the group the measurand belongs to
 * @return array with the syntax of possible interim results
 */
function get_interim_results($measurand_id, $group_id) {
	$array           = array();
	$names           = array();
	$interim_results = array();

	$names = get_possible_rra_names($group_id);
...
function get_possible_variables($template_id) {
	...
	$array = array_keys($measurand_ops_and_opds[6]);
	foreach ($names as $name) {
 ...
 */
function get_possible_data_query_variables($group_id) {

	// any data query associated with this data template?
	$sql = "SELECT DISTINCT(`data_local`.`snmp_query_id`) " .
			"FROM `data_local` " .
			"INNER JOIN `plugin_reportit_data_source_items` " .
			"ON `data_local`.`data_template_id` = `plugin_reportit_data_source_items`.`data_template_id` " .
			"WHERE `plugin_reportit_data_source_items`.`group_id`=$group_id";
...
function load_external_libs($name){
	...
	switch ($name) {
		case 'pclzip':
			...
			include_once( REPORTIT_INCLUDE_PATH . '/phppclzip/pclzip.lib.php');
		break;
		case 'graidle':

		break;
		case 'cleanXML':
			include_once( REPORTIT_INCLUDE_PATH . '/phppclxml/pclxml.lib.php');
...
function cache_xml_file($report_id, $mtime){
	...
	$sql = "REPLACE INTO plugin_reportit_tmp_" . $cache_id . " $columns VALUES $values;";
	db_execute($sql);
}
