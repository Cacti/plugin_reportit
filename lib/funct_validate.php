...
function validate_calc_formula($calc_formula, $calc_interim_results, $calc_var_names, $calc_data_query_variables) {
	global $measurand_ops_and_opds, $calc_fct_names, $calc_fct_names_params;

	//Valid signs:
	$valids['interim_results']['S']   	= $calc_interim_results;
	$valids['interim_results']['R']   	= 'E';
	$valids['functions']['S']    		= array_keys($measurand_ops_and_opds[0]);
	$valids['functions']['R']    		= 'E';
	$valids['fct_nwp']['S']      		= array_keys($measurand_ops_and_opds[1]);
	$valids['fct_nwp']['R']      		= 'P';
	$valids['fct_aliases']['S']      	= array_keys($measurand_ops_and_opds[2]);
	$valids['fct_aliases']['R']      	= 'P';
	...
	$valids['signs']['R']        		= array('L', 'R', 'L', 'R', '.', ',');
	$valids['operators']['S']    		= array('+','-','**','/','%','*');
	$valids['operators']['R']    		= array('S','S','O','O','O','O');
	...
	if ( (substr_count($calc_formula, '(') != substr_count($calc_formula, ')')) | (substr_count($calc_formula, '[') != substr_count($calc_formula, ']')) ) {
...
function validate_uploaded_template(){
	...
	if (isset($_FILES['import_file'])) {
		...
		$template_data = file_get_contents($_FILES['import_file']['tmp_name']);
	} else {
		session_custom_error_message('file', __('No file uploaded.'), false);
		return false;
	}
